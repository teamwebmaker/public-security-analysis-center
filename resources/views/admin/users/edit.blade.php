	...
	<x-admin.crud.form-container method="POST" insertMethod="PUT" action="{{ route($resourceName . '.update', $user) }}"
		...
		<div class="row">
			...
			<div class="col-md-6 mb-3 role-dependent" data-role="responsible_person" style="display: none;">
				<x-form.checkbox-dropdown label="სერვისებზე წვდომა" :items="$services" name="service_ids" labelField="title.ka"
					:selected="old('service_ids', $user->services->pluck('id')->toArray())" />
			</div>
		</div>
		...
		<div class="row">
			<div class="col-md-6 mb-3 role-dependent" data-role="worker" style="display: none;">
				<x-form.checkbox-dropdown label="სამუშაოს შემქმნელი კომპანია" :items="$companies" name="worker_company_ids"
					labelField="name" :selected="old('worker_company_ids', $user->workerCompanies->pluck('id')->toArray())" />
				<div class="form-text">თანამშრომელი სამუშაოს შექმნას <strong>მხოლოდ</strong> ამ კომპანიების ფილიალებში შეძლებს.
				</div>
			</div>
			<div class="col-md-6 mb-3 role-dependent" data-role="worker" style="display: none;">
