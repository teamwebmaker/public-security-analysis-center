...
@section('main')
	<x-admin.crud.form-container method="POST" insertMethod="PUT" title="სამუშაოს რედაქტირება"
		action="{{ route($resourceName . '.update', $task) }}" :backRoute="$resourceName . '.index'">
		...
		<div class="row">
			<div class="col-md-6 mb-3">
				<x-form.input type="datetime-local" name="start_date" label="საწყისი თარიღი"
					value="{{ old('start_date', $task->start_date) }}" min="{{ date('Y-m-d') }}" />

			</div>
			<div class="col-md-6 mb-3">
				<x-form.input type="datetime-local" name="end_date" label="დასასრული თარიღი"
					value="{{ old('end_date', $task->start_date) }}" min="{{ date('Y-m-d', ) }}" :required="false" />
			</div>
		</div>
