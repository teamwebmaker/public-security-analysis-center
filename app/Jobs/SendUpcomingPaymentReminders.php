...
class SendUpcomingPaymentReminders implements ShouldQueue
{
   ...
   public function handle(
      ...
      $reminderDate = now($businessTimezone)->addDays(2)->toDateString();
      $skippedSummary = [
      ...
      try {
         ...
         TaskOccurrence::query()
            ...
            ->chunkById(100, function ($occurrences) use ($smsLogService, $smsSender, $reminderDate, &$skippedSummary) {
               ...
               try {
                  ...
                  Log::error('Failed to send upcoming-payment SMS for chunk', [
                     'event_type' => $eventType,
                     'occurrence_ids' => $occurrenceIds,
                     'error' => $e->getMessage(),
                  ]);
         ...
         Log::error('SendUpcomingPaymentReminders failed unexpectedly', [
            'error' => $e->getMessage(),
         ]);
      }
      ...
      if ($this->hasSkippedSummary($skippedSummary)) {
         try {
            $messageStoreService->createAndDispatch([
               'source' => 'system',
               'subject' => 'გადახდის შეხსენება ვერ გაიგზავნა',
               'message' => $this->buildSkippedSummaryMessage(
                  'გადახდის შეხსენება ვერ გაიგზავნა შემდეგი მიზეზებით:',
                  $skippedSummary
               ),
            ]);
         } catch (\Throwable $e) {
            Log::error('Failed to store upcoming-payment system message', [
               'error' => $e->getMessage(),
            ]);
         }
      }
