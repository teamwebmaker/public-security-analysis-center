...
class TaskPresenter
{
   public static function format(Task $task, string $context = 'default'): array
   {
         ...
         return [
            ...
            'worker' => $task->users->count() === 1
               ...
               : ($task->users->count() >= 2
                  ...
                  '</select>'
                  : $badge('არ ჰყავს', 'danger')),
            'branch' => $task->branch_name ?? 'უცნობი',
            'service' => $task->service?->title->ka ?? $task->service?->title->en ?? 'უცნობი',
            'start_date' => e($task->start_date->diffForHumans()),
            'created_at' => e($task->created_at->diffForHumans()),
            'updated_at' => e($task->updated_at->diffForHumans()),
         ];
